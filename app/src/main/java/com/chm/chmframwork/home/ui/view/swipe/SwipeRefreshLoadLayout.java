...
/**
 * 自定义SwipeRefreshLayout,进行扩展上拉加载更多的功能
 * https://github.com/miomin/SHSwipeRefreshLayout
 * Created by chenmin on 2017/6/22.
 */
public class SwipeRefreshLoadLayout extends FrameLayout implements NestedScrollingParent {
    ...
    public interface OnRefreshListener {
        ...
        void onRefreshPullStateChange(float percent, int state);
    ...
    private GuidanceView headerView = new GuidanceView(getContext());
    private GuidanceView footerView = new GuidanceView(getContext());
    ...
    public SwipeRefreshLoadLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        //Indicator背景颜色
        resId = ta.getResourceId(R.styleable.SwipeRefreshLoadLayout_guidance_view_bg_color, -1);
        if (resId == -1) {
            mGuidanceViewBgColor = ta.getColor(R.styleable.SwipeRefreshLoadLayout_guidance_view_bg_color,
                    Color.WHITE);
        } else {
        ...
        //加载文字颜色
        resId = ta.getResourceId(R.styleable.SwipeRefreshLoadLayout_guidance_text_color, -1);
        if (resId == -1) {
            mGuidanceViewTextColor = ta.getColor(R.styleable.SwipeRefreshLoadLayout_guidance_text_color,
                    Color.BLACK);
        } else {
        ...
        //进度条背景颜色
        resId = ta.getResourceId(R.styleable.SwipeRefreshLoadLayout_progress_bg_color, -1);
        if (resId == -1) {
            mProgressBgColor = ta.getColor(R.styleable.SwipeRefreshLoadLayout_progress_bg_color,
                    Color.WHITE);
        } else {
        ...
        //进度条颜色
        resId = ta.getResourceId(R.styleable.SwipeRefreshLoadLayout_progress_bar_color, -1);
        if (resId == -1) {
            mProgressColor = ta.getColor(R.styleable.SwipeRefreshLoadLayout_progress_bar_color,
                    Color.RED);
        } else {
        ...
        //下拉刷新文字描述
        resId = ta.getResourceId(R.styleable.SwipeRefreshLoadLayout_refresh_text, -1);
        if (resId == -1) {
            mRefreshDefaulText = ta.getString(R.styleable.SwipeRefreshLoadLayout_refresh_text);
        } else {
            mRefreshDefaulText = resources.getString(resId);
        }

        //上拉加载文字描述
        resId = ta.getResourceId(R.styleable.SwipeRefreshLoadLayout_load_text, -1);
        if (resId == -1) {
            mLoadDefaulText = ta.getString(R.styleable.SwipeRefreshLoadLayout_load_text);
        } else {
        ...
        if (!canChildScrollUp() && mPullRefreshEnable && mCurrentAction == ACTION_PULL_REFRESH) {
            ...
            if (onRefreshListener != null) {
                if (lp.height >= guidanceViewHeight) {
                    onRefreshListener.onRefreshPullStateChange(lp.height / guidanceViewHeight, OVER_TRIGGER_POINT);
                } else {
                    onRefreshListener.onRefreshPullStateChange(lp.height / guidanceViewHeight, NOT_OVER_TRIGGER_POINT);
                }
            }
    ...
    private void handlerAction() {
        ...
        if (mPullRefreshEnable && mCurrentAction == ACTION_PULL_REFRESH) {
            ...
            if (lp.height >= guidanceViewHeight) {
                ...
                if (onRefreshListener != null)
                    onRefreshListener.onRefreshPullStateChange(1, START);
            } else if (lp.height > 0) {
